...
// 楂樼缁忓吀锛坕ndices 0-4锛?
const SKIN_CLASSIC = [
  { logo: '#9FE7E6', text: '#E8FBFF', bookmark: '#156B8C', book: '#052228', name: '骞借摑缁忓吀' },
  { logo: '#64A1C0', text: '#E7D2BF', bookmark: '#FF6357', book: '#2E2E2E', name: '澧ㄩ煹缁忓吀' },
  { logo: '#23A8A7', text: '#F46A6A', bookmark: '#053154', book: '#181818', name: '鏈辩爞鍙ゅ吀' },
  { logo: '#FF6054', text: '#FFDCAC', bookmark: '#5B889F', book: '#292929', name: '閲戠娴侀噾' },
  { logo: '#FCC96E', text: '#2E9C91', bookmark: '#136058', book: '#132432', name: '缈犵帀鍙ら煹' },
];
// 璐ㄦ劅鑺傛棩椋庯紙indices 5-19锛?
const SKIN_FESTIVE = [
  { logo: '#053154', text: '#FFFFFF', bookmark: '#5A5750', book: '#F05654', name: '闆鍒濋洩' },
  { logo: '#C7371D', text: '#FCDC93', bookmark: '#185A56', book: '#4D262B', name: '鏆栭槼濡傛矏' },
  { logo: '#F05654', text: '#D9AE84', bookmark: '#C0732F', book: '#200E02', name: '鏆簯娴侀噾' },
  { logo: '#D1A1BA', text: '#A6D4D6', bookmark: '#024B5E', book: '#841C3C', name: '鐜懓鍙ゅ牎' },
  { logo: '#EC9B7A', text: '#F8C761', bookmark: '#203822', book: '#595182', name: '鐒︾硸鏆栧繂' },
  { logo: '#A92A01', text: '#3E5626', bookmark: '#F4D376', book: '#F3CDA8', name: '妾€绾㈠嵃璁? },
  { logo: '#9B8037', text: '#9C2C21', bookmark: '#E4B0B7', book: '#FEC1AE', name: '璧孩鍗拌' },
  { logo: '#3A4B5B', text: '#061B3A', bookmark: '#F3CDA8', book: '#97D4F1', name: '鐏拌摑鏆壊' },
  { logo: '#B9502B', text: '#8D2922', bookmark: '#FFA279', book: '#FFC5CD', name: '鐮栫孩鐩愮郴' },
  { logo: '#145750', text: '#FCC96E', bookmark: '#212F3A', book: '#2D8A80', name: '缈犵豢娴侀噾' },
  { logo: '#9B8BA6', text: '#E3B0B7', bookmark: '#2A1943', book: '#881C3C', name: '绱獉鍙ら煹' },
  { logo: '#A85253', text: '#1C2C58', bookmark: '#CB7761', book: '#FFC4C2', name: '涓侀鏈堣壊' },
];
// 娓╂煍娲绘臣锛坕ndices 20-26锛屽叡7涓級
const SKIN_GENTLE = [
  { logo: '#B4CCD2', text: '#F0DC84', bookmark: '#F7F7E0', book: '#79A3CE', name: '鏄ユ棩鑺卞洯' },
  { logo: '#3D7695', text: '#C85E3D', bookmark: '#DDB355', book: '#DFC7B4', name: '鎱垫噿娴锋磱' },
  { logo: '#A4C2CA', text: '#B05553', bookmark: '#3B567F', book: '#CAA4A3', name: '澶嶅彜灏忛晣' },
  { logo: '#95C3BE', text: '#DA6790', bookmark: '#AEBE6C', book: '#ECB4A4', name: '娴极鑺卞' },
  { logo: '#C5E8E0', text: '#6B5B95', bookmark: '#F7CAC9', book: '#92A8D1', name: '绱綏鍏? },
  { logo: '#FFE4C4', text: '#2F5D62', bookmark: '#FFE4B5', book: '#5E8C7B', name: '鐞ョ弨妫灄' },
  { logo: '#B0C4DE', text: '#FFB6C1', bookmark: '#E6E6FA', book: '#DDA0DD', name: '钖拌。鑽変箣鎭? },
];
// 鐢滅編娓呮柊锛坕ndices 27-32锛屽叡6涓級
const SKIN_SWEET = [
  { logo: '#F9F4EA', text: '#8D6C63', bookmark: '#FCD3D5', book: '#CFE4DD', name: '钖勮嵎鏇兼尝' },
  { logo: '#B5DAE9', text: '#63BAD9', bookmark: '#CCE7D9', book: '#CAEBED', name: '鐡﹀皵鐧绘箹' },
  { logo: '#4663ac', text: '#f3dd87', bookmark: '#ee8984', book: '#79bfe0', name: '缇庡紡澶嶅彜' },
  { logo: '#B9D9EA', text: '#EFE4D4', bookmark: '#F8819B', book: '#F5C7C9', name: '鍏堝笣鏈€鐖? },
  { logo: '#E4F6A9', text: '#FF82A2', bookmark: '#BDDE93', book: '#FED0D6', name: '闈掓彁鑺箰' },
  { logo: '#CADEC3', text: '#225DAB', bookmark: '#CBAF98', book: '#95B4E0', name: '妞伴娴峰矝' },
  { logo: '#F1D996', text: '#779B84', bookmark: '#B69F8A', book: '#C8D4C0', name: '娴厜璺冮噾' },
  { logo: '#C5863C', text: '#FAE593', bookmark: '#849C4D', book: '#FE8F29', name: '姗樺瓙姹芥按' },
  { logo: '#EFE4D4', text: '#302B58', bookmark: '#9078AC', book: '#B6A9C8', name: '娴风洂钁¤悇' },
];
// 瀹屾暣鏁扮粍锛堜緵閫氱敤鏌ヨ〃锛宨ndices 0-32锛?
const BOOK_COLOR_SCHEMES = [...SKIN_CLASSIC, ...SKIN_FESTIVE, ...SKIN_GENTLE, ...SKIN_SWEET];

// 鏆撮湶 name 鏁扮粍渚?db.js 浣跨敤锛堥伩鍏嶇‖缂栫爜閲嶅锛?
...
// 閲嶆柊鏋勫缓 SKIN_DISPLAY_NAMES锛堣鐩?db.js 鍔犺浇鏃剁殑绌哄€硷級
...
const _t4WrongbookPage = {
  ...
  async renderGrid(container) {
    ...
    if (books.length === 0) {
      container.innerHTML = `<div class="empty-state"><i class="bi bi-book"></i><p>妗堜笂绌虹┖锛屽幓鑰冧竴鍦哄惂</p><a href="#/t1" class="btn-seal">鍘昏础闄?/a></div>`;
      return;
    }
    ...
    function renderBookCards() {
      ...
      for (var i = 0; i < books.length; i++) {
        ...
        html += '<div class="book-wrapper">' +
          ...
          '<div class="book-label" onclick="t4OpenSkinPopup(this,' + b.bankId + ')" title="鐐瑰嚮鏇存崲鐨偆"><i class="bi bi-palette-fill"></i><span>' + escapeHtml(BOOK_COLOR_SCHEMES[b.colorIndex != null ? b.colorIndex : i % BOOK_COLOR_SCHEMES.length].name) + '</span></div>' +
        '</div>';
      }
    ...
    container.innerHTML = '<div style="display:flex;flex-direction:column;gap:0.5rem;padding:0">' +
        '<p style="font-family:var(--font-hand);color:var(--ink-faint);margin:0 0 8px;letter-spacing:0.05em">姣忔湰涔︽敹褰曚竴鏈嵎瀹楃殑閿欓銆傜偣涔﹂槄鍗凤紝鐐瑰悕鐗屾崲鐨€?/p>' +
  ...
  async renderDetail(container, params) {
    ...
    if (!bank) { showToast('棰樺簱涓嶅瓨鍦?, 'error'); location.hash = '#/t4'; return; }
    ...
    setHeaderActions(`
      <a href="#/t4" class="btn-tag" style="text-decoration:none"><i class="bi bi-arrow-left"></i> 杩旈敊棰樻灦</a>
      <button class="btn-seal btn-seal-jade" onclick="t4OpenReviewModal(${bankId})"><i class="bi bi-lightbulb me-1"></i>鎵洸</button>
    `);

    const firstChar = (bank.name || '鍗?).trim().charAt(0) || '鍗?;

    // 棰樺瀷缁熻 (澶嶇敤 t2/view 鐨?TYPE_META)
    const TYPE_META = {
      single: { label: '鍗曢€?, color: 'var(--jade)', icon: '鈼? },
      multi:  { label: '澶氶€?, color: '#4A7B95',     icon: '鈼? },
      tf:     { label: '鍒ゆ柇', color: 'var(--gold)',  icon: '鈯? },
      fill:   { label: '濉┖', color: '#1B7A4E',     icon: '鈼? },
      essay:  { label: '闂瓟', color: '#7C3AED',     icon: '鉁? },
    };
    ...
    container.innerHTML = `
      <div class="bank-cover">
        ...
        <div class="bank-cover-text">
          <div class="bank-cover-eyebrow">閿? 棰? 褰? 路  鎷? 閬?/div>
          <h2 class="bank-cover-title">銆?{escapeHtml(bank.name)}銆嬮敊棰樻湰</h2>
          <p class="bank-cover-meta" id="t4DetailCount">鍏?<strong>${wrongQs.length}</strong> 閬?路 绱閿?<strong>${totalWrong}</strong> 娆?/p>
      ...
      <div class="t2-search-row">
        <div class="input-group">
          ...
          <input type="text" class="form-control" id="t4DetailSearch" placeholder="瀵婚敊 路 棰樺共鍏抽敭璇?.." value="${escapeHtml(t4DetailSearchQuery)}">
          <button class="btn-icon-jade" id="t4ClearSearch" title="娓呯┖" style="${t4DetailSearchQuery ? '' : 'display:none'}">
            <i class="bi bi-x"></i>
          </button>
        </div>
      </div>
        ...
        <section class="exam-main-col">
          ...
          <div class="exam-main-foot">
            <div class="exam-pager">
              <button class="btn-tag" id="t4DetailPrevBtn" ${t4DetailCurrentIndex === 0 ? 'disabled' : ''} title="涓婁竴棰?(鈫?"><i class="bi bi-chevron-left"></i> 涓婁竴棰?/button>
              <span class="exam-pager-counter" id="t4DetailPagerCount">${t4DetailCurrentIndex + 1} / ${wrongQs.length || 0}</span>
              <button class="btn-tag" id="t4DetailNextBtn" ${t4DetailCurrentIndex === (wrongQs.length - 1) ? 'disabled' : ''} title="涓嬩竴棰?(鈫?">涓嬩竴棰?<i class="bi bi-chevron-right"></i></button>
              <button class="btn-tag t4-remove-btn" id="t4DetailDeleteBtn" title="浠庨敊棰樻湰绉婚櫎姝ら"><i class="bi bi-trash"></i> 绉婚櫎</button>
              <span class="t2-pager-hint"><i class="bi bi-keyboard"></i> 鈫?/ 鈫?缈婚〉</span>
            </div>
          </div>
        </section>
      </div>`;
  ...
  _t4BindSearch(bankId) {
    ...
    const applyFilter = (rawQ) => {
      ...
      if (!q) {
        // Reset to full list 鈥?we need to keep a snapshot
  ...
  showT4DetailQuestion() {
    ...
    if (total === 0) {
      area.innerHTML = '<div class="text-center text-muted py-4">鏆傛棤閿欓</div>';
      const sidebar = document.getElementById('t4DetailSidebar');
      if (sidebar) sidebar.innerHTML = '';
      const countEl = document.getElementById('t4DetailCount');
      if (countEl) countEl.textContent = '鍏?0 閬撻敊棰?;
      return;
    }
    ...
    // 鍗烽潰 鈥?棰樺彿鍗帮紙甯﹂敊娆★級+ 棰樺共 + 绛旀鏈辨壒 + 瑙ｆ瀽绗?
    ...
    const explanationHtml = wq.explanation
      ? `<div class="q-explanation">
           <div class="q-explanation-label">瑙? 绗?/div>
    ...
    area.innerHTML = `
      <article class="q-page q-page--wrong">
        <header class="q-page-head">
          <div class="q-stamp q-stamp--wrong" aria-label="棰樺彿">
            <span class="q-stamp-num">${t4DetailCurrentIndex + 1}</span>
            <span class="q-stamp-label">绗?${t4DetailCurrentIndex + 1} 棰?/span>
          </div>
          <div class="q-page-meta">
            <span class="q-wrong-badge" title="${wrongCount} 娆＄瓟閿?{lastWrong ? '锛屾渶杩?' + lastWrong : ''}">
              <i class="bi bi-x-circle-fill"></i> 閿?${wrongCount} 娆?
            </span>
        ...
        ${answerText ? `<div class="q-answer-line">
          <span class="q-answer-mark">绛?/span>
    ...
    if (delBtn) {
      delBtn.addEventListener('click', async () => {
        ...
        if (countEl) {
          ...
          countEl.innerHTML = `鍏?<strong>${t4DetailAllQuestions.length}</strong> 閬?路 绱閿?<strong>${totalWrong}</strong> 娆;
        }
  ...
  renderT4DetailSidebar(container) {
      ...
      html += `<div class="type-section${collapsed}" data-type="${qtype}">
        ...
        <div class="type-body">
          ${questions.map(q => {
            ...
            return `<div class="${cssClass}" data-qid="${q.questionId}" title="#${q.number} 鈥?閿?{q.wrongCount || 1}娆?>${q.number}</div>`;
          }).join('')}
        </div>
      </div>`;
    }
    ...
    container.innerHTML = `
      <header class="t2-toc-head">
        <div class="t2-toc-eyebrow">閿? 棰? 褰?/div>
        <div class="t2-toc-total">鍏?<strong>${totalInBook}</strong> 路 閿?<strong>${totalWrong}</strong> 娆?/div>
      </header>
      <div class="exam-sidebar-questions">${html}</div>
    `;

    // Circle click handlers 鈥?navigate to the clicked question
  ...
  async renderReview(container, params) {
    const bankId = parseInt(params.bankId);
    const bank = await getBankById(bankId);
    setState({ _bankName: bank ? bank.name : '棰樺簱' });
  ...
  async renderReviewUI(container, bankId) {
    ...
    container.innerHTML = `
      ...
      <div class="d-flex align-items-center gap-3 mb-3" id="t4StatusBar">
        <span class="badge bg-success fs-6"><i class="bi bi-check-circle me-1"></i>宸叉竻闄?${engine.cleared} 棰?/span>
        <span class="badge bg-secondary fs-6"><i class="bi bi-hourglass-split me-1"></i>鍓╀綑 ${totalRemaining} 棰?/span>
      </div>

      <div id="t4ReviewArea"></div>

      <div class="d-flex justify-content-between mt-3">
        <a href="#/t4/${bankId}" class="btn btn-outline-danger"><i class="bi bi-box-arrow-left me-1"></i>閫€鍑烘壂鐩?/a>
      </div>`;
  ...
  renderCurrentQuestion() {
    ...
    if (!engine || engine.group.length === 0) {
      area.innerHTML = `<div class="text-center py-5">
        ...
        <h5 class="mt-2">鎵洸瀹屾垚锛?/h5>
        <p class="text-muted">鍏辨竻闄や簡 ${engine ? engine.cleared : 0} 閬撻敊棰?/p>
        <a href="#/t4/${location.hash.split('/')[2]}" class="btn btn-primary">杩斿洖閿欓鏈?/a>
      </div>`;
      // Update status bar to show final result
      const statusEl = document.getElementById('t4StatusBar');
      if (statusEl) {
        statusEl.innerHTML = `<span class="badge bg-success fs-6"><i class="bi bi-trophy me-1"></i>鍏辨竻闄?${engine ? engine.cleared : 0} 棰?/span>`;
      }
      return;
    }
    ...
    if (isResult) {
      ...
      answerHtml += `<div class="alert ${isCorrect ? 'alert-success' : 'alert-danger'} py-2 text-center">${isCorrect ? '<i class="bi bi-check-circle me-1"></i>鍥炵瓟姝ｇ‘锛? : '<i class="bi bi-x-circle me-1"></i>鍥炵瓟閿欒'}</div>`;

      if (!isCorrect) {
        answerHtml += `<div class="p-2 rounded mb-2" style="background:#e6f9f3;color:#0d6b3c;"><strong>姝ｇ‘绛旀锛?/strong>${formatCorrectAnswer(correctAnswer, type)}</div>`;
      }
      ...
      answerHtml += `<div class="text-center mt-3">
        <button class="btn btn-primary btn-lg" onclick="t4NextQuestion()">涓嬩竴棰?<i class="bi bi-arrow-right"></i></button>
      </div>`;
    } else {
      ...
      if (type === 'multi' || type === 'fill' || type === 'essay') {
        answerHtml += `<div class="text-center mt-3">
          <button class="btn btn-success btn-lg" id="t4SubmitBtn" onclick="t4SubmitAnswer()"><i class="bi bi-check-lg me-1"></i>鎻愪氦绛旀</button>
        </div>`;
      }
    }
      ...
      <div class="card">
        <div class="card-body p-4">
          <div class="d-flex justify-content-between align-items-start mb-3">
            <div>
              ${typeBadge(wq.type)}
              <span class="badge bg-warning text-dark ms-1">閿欒 ${wq.wrongCount} 娆?/span>
            </div>
            <div class="d-flex align-items-center gap-2">
              ...
              <button class="btn btn-sm btn-outline-primary btn-icon" onclick="t4SkipQuestion(${idx})" title="鏆傛椂璺宠繃"><i class="bi bi-skip-forward"></i></button>
            </div>
          </div>
  ...
  renderReviewAnswerInput(wq, idx, readOnly, isCorrect, correctAnswer) {
    ...
    switch (type) {
      ...
      case 'tf': {
        // Show 鉁?鉁?instead of true/false; fixed order: 姝ｇ‘ first, 閿欒 second
        const opts = { 'true': '姝ｇ‘', 'false': '閿欒' };
        ...
        return `<ul class="options-list">${labels.map(l => {
          ...
          return `<li class="${itemClass}" data-value="${l}"><span class="option-letter">${l === 'true' ? '鉁? : '鉁?}</span><span>${opts[l]}</span></li>`;
        }).join('')}</ul>`;
      }
      case 'multi': {
        const opts = wq.options || {};
      ...
      case 'fill': {
        ...
        return blanks.map((ans, i) => {
          if (readOnly) {
            ...
            return `<div class="mb-1"><label class="form-label small fw-bold">绗?{i+1}绌?/label>
              <div><span class="${cls}">浣犵殑绛旀锛?{escapeHtml(ua[i] || '(鏈綔绛?')}</span></div>
              ${!correct ? `<div class="text-success">姝ｇ‘绛旀锛?{escapeHtml(ans)}</div>` : ''}
            </div>`;
          }
          return `<div class="mb-1"><label class="form-label small fw-bold">绗?{i+1}绌?/label><input type="text" class="form-control form-control-sm review-input" data-blank="${i}"></div>`;
        }).join('');
      }
      case 'essay': {
        if (readOnly) {
          return `<div>
            <div><span class="${isCorrect ? 'text-success' : 'text-danger'}">浣犵殑绛旀锛?{escapeHtml(t4ReviewEngine._lastUserAnswer || '(鏈綔绛?')}</span></div>
            ${!isCorrect ? `<div class="text-success">姝ｇ‘绛旀锛?{escapeHtml(correctAnswer)}</div>` : ''}
          </div>`;
        }
        return `<textarea class="form-control review-input" rows="3" placeholder="璇疯緭鍏ョ瓟妗?></textarea>`;
      }
      default:
        return '';
    }
...
async function t4OpenReviewModal(bankId) {
  ...
  let modalHtml = '<p class="mb-3">閫夋嫨瑕佹壂鐩茬殑棰樺瀷锛?/p>';
  for (const t of types) {
    const count = allWQs.filter(w => w.type === t).length;
    modalHtml += `<div class="form-check"><input class="form-check-input t4-review-type" type="checkbox" value="${t}" id="rt_${t}" checked><label class="form-check-label" for="rt_${t}">${TYPE_LABELS[t]}锛?{count} 棰橈級</label></div>`;
  }

  const { result } = showModal('閫夋嫨鎵洸棰樺瀷', modalHtml, [
    { text: '鍙栨秷', cls: 'btn-secondary' },
    { text: '寮€濮嬫壂鐩?, cls: 'btn-success' }
  ]);
  ...
  if (idx === 1) {
    ...
    if (selectedTypes.length === 0) { showToast('璇疯嚦灏戦€夋嫨涓€绉嶉鍨?, 'warning'); return; }
...
async function t4SubmitAnswer() {
  ...
  if (isCorrect) {
    ...
    if (wq.correctStreak >= 3) {
      ...
      emit('coins:updated');
      showToast('宸叉帉鎻★紒杩炵画姝ｇ‘3娆★紝+1 纭竵', 'success');
    } else {
      showToast(`姝ｇ‘锛佽繕闇€杩炵画姝ｇ‘ ${3 - wq.correctStreak} 娆, 'success');
    }
  } else {
    wq.correctStreak = 0;
    wq.wrongCount++;
    await putWrongQuestion(wq);
    showToast('鍥炵瓟閿欒锛岃繛缁纭鏁板凡閲嶇疆', 'warning');
  }
